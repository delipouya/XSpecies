...
pdf(paste0("Results/",INPUT_NAME,'/labeling_',OUTPUT_NAME,'.pdf'))

labeling_df_SCINA<- data.frame(SCINA_label=as.character(SCINA_res$cell_labels),
                         tSNE_1 = getEmb(seur, 'tsne')[,1],
                         tSNE_2 = getEmb(seur, 'tsne')[,2])
p_scina = ggplot(labeling_df_SCINA, aes(x=tSNE_1, y=tSNE_2, color=SCINA_label))+geom_point(alpha=0.6)+theme_bw()+
  ggtitle(paste0(INPUT_NAME, ' SCINA results'))+scale_colour_brewer(palette = "Paired")
print(p_scina)


names(Cell_type_assigned) <- gsub("\\s*\\([^\\)]+\\)","",names(Cell_type_assigned))
labeling_df_AUCELL<- data.frame(AUCELL_label=as.character(get_vector_of_lables(Cell_type_assigned, seur)),
                               tSNE_1 = getEmb(seur, 'tsne')[,1],
                               tSNE_2 = getEmb(seur, 'tsne')[,2])
p_aucell= ggplot(labeling_df_AUCELL, aes(x=tSNE_1, y=tSNE_2, color=AUCELL_label))+geom_point(alpha=0.5)+
  theme_bw()+ggtitle(paste0(INPUT_NAME, ' AUCELL results'))
print(p_aucell)
...
# pdf(paste0(AUCell_dir,'/labeling_',OUTPUT_NAME,'.pdf'), height = 7, width = 7)
...
#   labeling_df<- data.frame(AUCell=as.numeric(cells_AUC_df[index,]),
#                            AUCell_label=ifelse(colnames(seur) %in% marked_cells,marked_cells_name,'other'),
#                            tSNE_1 = getEmb(seur, 'tsne')[,1],
#                            tSNE_2 = getEmb(seur, 'tsne')[,2])
#   p1=ggplot(labeling_df, aes(x=tSNE_1, y=tSNE_2, color=AUCell_label))+geom_point(alpha=0.6)+theme_bw()+
#     ggtitle(paste0(marked_cells_name, ' AUCell results'))+
#     scale_color_manual(values = c('tomato2', 'cyan3'))
#   
# }
# dev.off()
